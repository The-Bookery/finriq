...
export const execute = async (client, message, args) => {
  ...
  if (!args || args == "")
    return message.channel.send(
      "You must include a name, a role, and a description, seperated by semicolons."
    );
  ...
  if (args.length < 3) {
    if (!args[0] || args[0] == "")
      return message.channel.send("You must include a name!");
    if (!args[1] || args[1] == "")
      return message.channel.send(
        "You must include a role! Make sure you use either it's name or ID."
      );
    if (!args[2] || args[2] == "")
      return message.channel.send("You must include a description!");
    return;
  }
  ...
  if (description == "")
    return message.channel.send("Club's description cannot be empty!");
