...
export = async (client, member) => {
  ...
  welcomeDM.fields = [
    {
      name: "Gain Access",
      value: "If you haven't already, be sure to verify your email and read the rules. You can find the rules popup by clicking the `Complete` button to the right of the message bar on the bottom.",
      inline: false
    },
    {
  ...
  member.send({embeds: [welcomeDM]}).catch((err) => {
    if (err.name == "DiscordAPIError") {
      welcomeDM.fields.push({
        name: "Note",
        value: "This was sent in this channel, likely because you have disabled DMs from servers. This will be automatically deleted after one minute.",
        inline: true
      });
