...
export = async (client, oldMember, newMember) => {
  if (oldMember.pending && !newMember.pending) {
    ...
    const ranksrole = newMember.guild.roles.cache.find(
      (role) => role.id === config.roles.ranksDivider
    );
    const inforole = newMember.guild.roles.cache.find(
      (role) => role.id === config.roles.infoDivider
    );
    const genrerole = newMember.guild.roles.cache.find(
      (role) => role.id === config.roles.genreDivider
    );
    const pingsrole = newMember.guild.roles.cache.find(
      (role) => role.id === config.roles.pingsDivider
    );
    try {
